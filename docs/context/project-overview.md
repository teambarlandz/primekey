# Project Overview for Primekey Homes and Properties Ltd Web App
## Overview

This application is a self-hosted digital sales engine for a Nigerian real estate company called Primekey Homes and Properties Ltd., Primekey for short. It is designed to acquire, qualify, and convert high-intent leads across multiple user segments within a single, unified platform. The company owns and controls all data, business logic, and infrastructure — with no reliance on external SaaS providers like HubSpot, Salesforce, or Shopify.
...
**Marketing Landing Page & Auth Gate:** A visitor arrives on the full marketing homepage (`/`), which features a Hero, Social Proof, Benefits, FAQ, and Final CTA. Pathway selection happens via the Navbar or inline CTAs. When a user clicks a business CTA (e.g., "Buy/Rent a Property" or "List a Property"), they hit an Authentication Gate.

**Buyer/Renter Journey (Phase 1):**
1. User authenticates (or signs up) and is routed to the Search Portal (`/search`).
2. User applies search filters (location, price, bedrooms, etc.) against the internal `properties` database.
3. **IF matching properties are found**, the user views listings, clicks for details, and can contact an agent (generating a standard lead).
4. **IF no properties match the search criteria**, the system conditionally triggers the "2-Week Concierge Search" offer modal instead of a dead-end "no results" page.
5. User registers for the concierge service by submitting a detailed form (name, contact, target location, budget, property specs) with explicit NDPR consent.
6. Submission creates a HIGH priority lead in the custom CRM, triggers an internal SLA alert for agent follow-up within 2 hours, and logs a timestamped consent record.

**Landlord/Owner Journey (Phase 2):**
1. User authenticates (or signs up) and is routed to the Landlord Dashboard/Landing page (`/landlord`).
2. User views the value proposition emphasizing the benefits of outsourcing property management (stress-free management, guaranteed rent, expert valuation).
3. User completes a detailed, gated property intake form (no free uploads allowed for unauthenticated users).
4. Upon completion, the system integrates with an automated appointment booking engine, allowing the user to schedule a consultation with an agent.
5. The landlord's details and property information are created as a new Opportunity record in the CRM, tagged for the landlord acquisition team.

## Features

### Phase 1: Buyer/Renter Pathway (Lead Generation Engine)
- **Marketing Landing Page:** A high-conversion, full-funnel marketing surface (Hero, Social Proof, Benefits, FAQ, Final CTA) that educates visitors and drives them to authenticate.
- **Authentication Gate:** Strict gating of all business functionality. Guests can view the marketing site, but must sign up/login to search, list, or submit forms.
...
- **Buyer/Renter Pathway (Phase 1):** Marketing Landing Page, Auth Gate, dynamic property search, "Not Found" conditional logic, "2-Week Concierge" registration with SLA enforcement, lead scoring, agent dashboard.
...
- Unified frontend with a Marketing Landing Page and Auth-Gated functional pathways for both Buyer and Landlord contexts.
...
- **Concierge Lead Conversion Rate:** At least 15% of users who encounter a "no results" search successfully register for the "2-Week Concierge Search" service.
- **SLA Adherence:** 95% of concierge leads trigger an internal alert and are acknowledged by an agent within 2 hours of submission.
- **Landlord Qualification Rate:** 70% of completed landlord intake forms successfully convert into a scheduled consultation appointment.
- **NDPR Audit Readiness:** A mock regulatory audit of the system confirms that consent logs are immutable, complete (timestamp, policy version), and that data export and hard-delete endpoints function as specified.
- **Performance Benchmark:** The core search and concierge submission APIs maintain a p95 latency of under 200ms under expected load.
- **Agent Productivity:** Agents report a measurable increase in lead response time and conversion efficiency using the dashboard.
...
- **2-Hour SLA for Concierge Leads:** Any lead submitted via the "2-Week Concierge Search" must have `priority='HIGH'` and trigger an internal notification immediately.
- **Search-First, Not Found Fallback:** An empty search result is the explicit trigger for the "Concierge Offer" modal — the system must never show a generic "no results found" page.
